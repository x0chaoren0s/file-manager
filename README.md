...
- **文件预览**：
  ...
  - **图片预览**：支持主流图片格式。
- **快速部署**：提供 `deploy.sh` 一键部署脚本，支持自定义端口、域名及子路径配置，深度适配**宝塔面板** (BT Panel) 环境，并支持**一键开启 HTTPS/SSL**。
...
| :--- | :--- | :--- |
| **资源开销** | **极低**。单 HTML 文件 (25KB)，Nginx 驱动，近乎零内存/CPU 占用。 | **较高**。需要 Python 后台及大量依赖，内存占用从几百 MB 起步。 |
...
| **阅读体验** | **专注**。专业编辑器风格预览窗，支持行号防拷贝与自动换行。 | **综合**。界面复杂，包含大量内核状态与执行控件。 |
